...
def _active_effect_rows(
    ...
    *,
    ...
    if effect in {"prism_bank_per_answer_1_5", "prism_bank_per_answer_1"}:
        ...
        progress_copy = (
            (
                ...
                else "+1 Growth banked per eligible card · Today’s Prism bank released"
            )
            if released else
    ...
    if fertilizer_batches:
        ...
        rows.append((
            f"{_fertilizer_display_name(tier)} · {plural_cards(remaining_cards)} left",
    ...
    if target is not None:
        ...
        if track is not None and getattr(track, "target", None) == target:
            ...
            claim_status = (
                ...
                else "Funded · Garden Coins needed to claim"
            ...
            return GrowthDestinationProjection(
                ...
                heading=heading,
                detail=" · ".join(
                    value for value in (status, claim_status, progress) if value
                ),
                route_copy=(
