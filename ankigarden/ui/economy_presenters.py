...
from .copy import (
    fertilizer_duration_copy,
    fertilizer_effect_copy,
...
def _player_card_copy(value: Any) -> str:
    """Keep catalog mechanics intact while hiding internal answer eligibility."""

    text = str(value or "")

    def card_term(match: re.Match[str]) -> str:
        noun = str(match.group(1) or "").casefold()
        replacement = "cards" if noun.endswith("s") else "card"
        return (
    ...
    text = re.sub(
        r"\beligible(?:\s+committed)?\s+(card(?:\s+answers?)?|answers?)\b",
        card_term,
        text,
        flags=re.IGNORECASE,
    )
    text = re.sub(r"\banswers\b", "cards", text, flags=re.IGNORECASE)
    text = re.sub(r"\banswer\b", "card", text, flags=re.IGNORECASE)
    return learner_card_copy(text).strip()


def _consumable_effect_copy(item: Any) -> str:
    growth_per_card = max(0, int(getattr(item, "growth_per_card", 0) or 0))
    card_count = max(0, int(getattr(item, "card_count", 0) or 0))
    if growth_per_card and card_count:
        return (
            f"{fertilizer_effect_copy(growth_per_card)} · "
            f"{fertilizer_duration_copy(card_count)}"
        )
    return _player_card_copy(getattr(item, "effect_description", ""))
...
def catalog_item_projections() -> tuple[CatalogItemProjection, ...]:
    ...
    for item in CONSUMABLES:
        ...
        rows.append(CatalogItemProjection(
            ...
            str(item.display_name),
            "consumable",
            _consumable_effect_copy(item),
            _player_card_copy(item.how_to_acquire),
    ...
    for category, definitions, id_field in (
        ...
        for item in definitions:
            ...
            rows.append(CatalogItemProjection(
                ...
                category,
                (
                    garden_bonus_effect_copy(item_id, item.effect_description)
                    if category == "garden_bonus"
                    else _player_card_copy(item.effect_description)
                ),
                _player_card_copy(item.how_to_acquire),
                str(item.asset_id),
                bool(item.purchasable),
    ...
    for item in LANDMARKS if landmarks_enabled() else ():
        ...
        rows.append(CatalogItemProjection(
            ...
            _player_card_copy(item.effect_description),
            _player_card_copy(item.how_to_acquire),
            str(item.asset_id),
            False, None,
        ))
