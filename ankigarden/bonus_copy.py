"""Concise decoration and scenery descriptions shared by every presentation."""

GARDEN_BONUS_EFFECT_COPY = {
    "seedling_sign": "No study bonus",
    "default": "No study bonus",
    "watering_station": "+1 Growth every 2 cards.\nFirst 200 cards each day.",
    "wind_chime": "+1 Growth every 5 cards.",
    "firefly_lantern": "+3 Growth every 5 cards to the unfinished plant closest to a checkpoint.",
    "autumn": "Finish today’s cards: +4 Coins.\n+50% Coins from checkpoints and growth stages.",
    "snowy": "1 Small Growth Charge every 2 days you finish today’s cards.",
    "full_moon": "While equipped: 1 Booster Potion every 6 completed days.\nOnce owned: new Potions last 25 extra cards.",
    "herbalist_hourglass": "While equipped: 1 Booster Potion every 30 completed days.\nOnce owned: new Potions last 25 extra cards.",
    "prism_trellis": "Save 1 Growth per card: first 100 each day, up to 300 saved.\nFinish today’s cards to collect it.",
    "spring": "+2 Growth per card.\nFirst 20 cards each day.",
    "summer": "+1 Growth every 2 cards.\nFirst 120 cards each day.",
    "harvest_bell": "Finish today’s cards: +5 Coins.",
    "rainbow_horizon": "+1 Growth per card.\nFirst 75 cards each day.",
    "halloween": "Finish today’s cards for 1 gift:\nSmall Growth Charge 95% · Standard Growth Charge 4% · Booster Potion 1%.",
    "eclipse": "+1 Growth per card.\nFirst 125 cards each day.",
}
