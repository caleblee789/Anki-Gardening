...
class AnkiGardenApp:
    ...
    def open_collection(
        self,
        collection_tab: str = "plants",
        *,
        focus_tier_id: str = "",
    ) -> None:
        """Open Collection, optionally deep-linked to one Landmark tier."""

        self._settings_open_pending = False
        self._starter_open_pending = False
        self._dashboard_select_plant_pending = False
        requested_tab = str(
            getattr(collection_tab, "value", collection_tab) or "plants"
        )
        normalized_tab = (
            "garden-landmarks"
    ...
    def open_garden_landmarks(self, *, focus_tier_id: str = "") -> None:
        ...
        self.open_collection(
            "garden-landmarks",
            focus_tier_id=str(focus_tier_id or ""),
        )
    ...
    def _open_dashboard_when_ready(self) -> None:
        ...
        try:
            ...
            if opening_settings:
                ...
                collection_tab = str(
                    getattr(self, "_collection_tab_pending", "plants")
                    or "plants"
                )
                focus_tier_id = str(
                    getattr(
                        self,
                        "_collection_focus_tier_id_pending",
                        "",
                    )
                    or ""
                )
                if collection_tab == "plants" and not focus_tier_id:
                    open_collection()
                else:
                    open_collection(
                        collection_tab,
                        focus_tier_id=focus_tier_id,
                    )
                self._collection_open_pending = False
                self._collection_tab_pending = "plants"
                self._collection_focus_tier_id_pending = ""
