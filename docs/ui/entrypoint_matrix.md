...
| Full Garden | **Garden Progress** | Opens Garden Progress on its primary page; Collection remains available in the same window. Collection keeps `10 of 10 species discovered` separate from `30 of 39 collection entries discovered`. | Opening/closing is transient and changes no persisted information. |
...
| Full Garden | **Settings** | Opens Garden naming, the current-style card, the structured Scenery/Displayed decoration/Active garden bonus/Visual effects summary, applicable Garden display/notification choices including the persistent reviewer HUD, active major reward reveals, and the presentation-only Sync Rewards receipt, plus explicit Save/Cancel controls without duplicating the home preview. Environment selectors, quality/detail/performance, animation, and Fine tune choices are absent. | Existing settings remain active if saving fails, the name limit is violated, or the learner cancels. |
