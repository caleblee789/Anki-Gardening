...
| Home card | Render once in Deck Browser and Overview; Open Garden and Refresh use scoped bridge commands; visibility and dashboard mutations refresh immediately; transient states remain padded and accessible. | Home-state, injection, bridge, accessibility, fallback, and visibility tests. | Both home surfaces, enable/disable, refresh, open, and post-mutation agreement. |
| First-use guidance | Explain review growth before the first answer, explain Nurture after it, and retire inline guidance after interaction or dismissal. Existing users do not receive repeated onboarding. | Presentation-state, config migration, wording, accessibility, and persistence tests. | Fresh launch, one real review, mouse/keyboard plant interaction, dismissal, and restart persistence. |
...
| Settings | Daily goal, home visibility, theme, quality, animation, and weather detail validate and save transactionally in a responsive, scrollable surface. | Configuration validation/rollback, responsive-breakpoint, and home visibility tests. | Change every control, inspect wide/narrow layouts, force refresh, restart, verify persistence. |
