...
| Plant composition | Six explicit background/terrace bed anchors plus slot order | One-to-six plants remain grounded, collision-free, and depth ordered in both the dashboard and home widget; unlocking a bed never changes an existing plant anchor and missing artwork keeps a named stage-appropriate fallback. |
...
| Plant interaction | Stable plant ID plus painted selection and native Qt actions | The scene selects and optionally drags plants; move mode uses compact Current, Move, Swap, and Locked badges from dedicated non-plant anchors, while accessible destination descriptions remain independent of painted copy. A focusable compact action bar provides Nurture, Move, Story, Cancel move, and status semantics. |
| Nurtured plant | Valid `focus_plant_id` | A grounded glow/ring identifies it without covering the artwork; the native action panel shows a passive Nurturing status in place of its action for the current recipient, and invalid IDs repair to the first slot. |
| First-use guidance | Review totals plus versioned Anki configuration | A fresh garden explains the review-to-growth loop; after the first review it prompts plant selection and Nurture, then retires after interaction or dismissal without changing garden state. |
