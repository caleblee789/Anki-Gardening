...
def test_initial_motion_plan_counts_from_zero_and_emphasizes_full_bloom_once() -> None:
    ...
    assert set(metrics) == {"Reviews", "Growth", "Garden Coins"}
    assert metrics["Reviews"].start_scaled == 0
    assert metrics["Reviews"].final_text == "1,234"
...
def test_update_motion_plan_contains_only_changed_metrics_progress_and_events() -> None:
    ...
    assert set(metrics) == {"Reviews", "Garden Coins"}
    assert metrics["Reviews"].start_scaled == 1_234
    assert metrics["Reviews"].end_scaled == 1_242
