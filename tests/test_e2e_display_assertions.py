...
def test_journey_load_home_to_dashboard_displays_exact_seeded_kpis(monkeypatch):
    ...
    assert 'data-testid="home-support" title="Aster · Seed · 250 / 500"' in html
...
def test_journey_review_session_then_refresh_persists_exact_values(monkeypatch):
    ...
    assert 'data-testid="home-support" title="Aster · Seed · 250 / 500"' in before
    ...
    assert 'data-testid="home-support" title="Aster · Seed · 310 / 500"' in updated
    assert "9-day streak" not in updated
    assert 'data-testid="home-weather"' not in updated

    assert 'data-testid="home-support" title="Aster · Seed · 310 / 500"' in refreshed
...
def test_journey_navigation_between_home_contexts_keeps_values_without_duplication(monkeypatch):
    ...
    for rendered in (deck_content.body, overview_content.body):
        ...
        assert 'data-testid="home-support" title="Aster · Seed · 250 / 500"' in rendered
