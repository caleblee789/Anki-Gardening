...
OUTPUT = ROOT / "build" / "twilight-rose-pilot"
RUNTIME_PLANTS = ROOT / "ankigarden" / "assets" / "v6_storybook_gouache" / "plants" / "rose"
...
def _stage_matrix(rows: list[dict]) -> Path:
    tile_size = (480, 300)
    sheet = Image.new(
        "RGB",
    ...
    path = OUTPUT / "rose_responsive_stage_matrix.png"
    sheet.save(path, "PNG", optimize=True)
    return path


def _progression_strip(rows: list[dict]) -> Path:
    tile_size = (640, 400)
    ...
    path = OUTPUT / "rose_growth_progression_all_layouts.png"
    sheet.save(path, "PNG", optimize=True)
    return path


def _alpha_strip() -> Path:
    tile_size = (300, 300)
    ...
    for column, stage in enumerate(STAGES):
        ...
        sheet.paste(tile, (column * tile_size[0], 0))
    path = OUTPUT / "rose_alpha_and_growth_line.png"
    sheet.save(path, "PNG", optimize=True)
    return path
...
def main() -> None:
    ...
    outputs = [_alpha_strip(), _progression_strip(rows), _stage_matrix(rows)]
    for path in outputs:
